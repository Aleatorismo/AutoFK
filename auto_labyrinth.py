"""
===================================================================================
                        AFK Journey 异界迷宫自动化脚本
===================================================================================

脚本用途：
    自动化执行 AFK Journey 游戏中的异界迷宫探索，通过 OCR 文字识别和自动点击
    来完成整个迷宫探索流程，包括路径选择、战斗、烙印选择等操作。

主要功能：
    1. 游戏窗口检测和截图
    2. 中文 OCR 文字识别 (基于 PaddleOCR)
    3. 智能文字定位和自动点击
    4. 完整的异界迷宫探索流程自动化

运行逻辑 (13个主要步骤)：
    步骤 1:  等待并点击"进入"按钮，处理可能的提醒对话框
    步骤 2:  检测"英雄"字样，点击预设位置进入迷宫
    步骤 3:  处理附加挑战界面，执行战斗-跳过-回放流程
    步骤 4:  检查进度 (13/15)，决定后续流程分支
    步骤 5:  选择路径 (优先级: 馈赠>晶粹>遗物>精英>道具)
    步骤 6:  执行战斗流程，包含智能"跳过"重试机制
    步骤 7:  遗物选择循环，支持多次选择直到完成
    步骤 8:  点击"烙印晶球"进入烙印替换界面
    步骤 9:  烙印替换逻辑 (优先替换精英>稀有品质)
    步骤 10: 基于步骤9结果的条件性烙印选择
    步骤 11: 点击"继续前进"，定位并选择首领挑战
    步骤 12: 首领战斗流程
    步骤 13: 探索结束处理，包含多轮烙印选择循环

核心特性：
    • 相对坐标系统：所有点击位置基于窗口大小的比例，适应不同分辨率
    • 智能 OCR 识别：支持中文文字识别，可配置置信度阈值
    • 容错机制：包含超时处理、重试逻辑和错误恢复
    • 调试模式：支持从指定步骤开始执行，便于测试和调试
    • 状态记忆：记录关键操作状态，影响后续决策逻辑

使用前配置：
    1. 调整 GAME_WINDOW_TITLE 为实际游戏窗口标题
    2. 根据游戏界面调整各项坐标比例常量
    3. 设置 DEBUG_START_STEP 进行调试 (可选)
    4. 确保 PaddleOCR 环境正确安装

依赖库：
    - PaddleOCR: 中文 OCR 识别引擎
    - win32gui/win32api: Windows 窗口操作和鼠标点击
    - mss: 高效屏幕截图
    - PIL/numpy: 图像处理
